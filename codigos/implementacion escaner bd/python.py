...
def on_key_press(key):
    ...
    try:
        ...
        if key == keyboard.Key.enter:  # Detectar Enter como fin de escaneo
            entry_id.delete(0, tk.END)  # Borrar cualquier valor previo
            entry_id.insert(0, current_code)  # Insertar el código escaneado en la casilla de ID
...
def insertar():
    ...
    sql = "INSERT INTO misdatos (numero_colaborador, nombres, apellidos, telefono, email) VALUES (%s, %s, %s, %s, %s)"
...
def editar():
    conexion = conectar()
    cursor = conexion.cursor()
    sql = "UPDATE misdatos SET numero_colaborador=%s, nombres=%s, apellidos=%s, telefono=%s WHERE email=%s"
    valores = (entry_id.get(), entry_nombre.get(), entry_apellido.get(), entry_telefono.get(), entry_email.get())
    try:
        cursor.execute(sql, valores)
        conexion.commit()
...
def eliminar():
    conexion = conectar()
    cursor = conexion.cursor()
    sql = "DELETE FROM misdatos WHERE numero_colaborador=%s"
    try:
...
# Iniciar la escucha de teclado en segundo plano
